	...
	<div class="home">
		<div class="panel fluid">
			...
			<div v-if="discoveryWorks.length > 0" class="container">
				<router-link tag="div" :to="{ name: 'photo.get', params: { id: w.id } }" v-for="(w, index) in discoveryWorks" :key="w.id" class="card-work">
					<img class="photo" :src="w.photo" height="200">
					<p>name: {{ w.name }}</p>
					<p>detail: {{ w.detail }}</p>
					<p>tags: {{ w.tags.join(',') }}</p>
					<p>post at: {{ w.createdAt | datetime }}</p>
					<div class="favorite" :class="{ active: w.isFavorite }" @click.stop="favorite(index)">
...
export default {
	...
	methods: {
		favorite (index) {
			...
			const w = this.discoveryWorks[index]
			this.isSaving = true
			this.$api.work.favorite({ id: `${w.id}`, favorite: !w.isFavorite })
				.then((resp) => {
					if (!resp.ok) {
						alert(resp.error.message)
						return
